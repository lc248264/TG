<template>
    <div class="views-fankui-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 反馈查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="fankuiren" label="反馈人" width="80">
                        <template #default="{row}"> {{ row.fankuiren }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="回复" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/fankui/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canFankuiSelect, useFankuiSelect, canFankuiDelete } from "@/module";
    ...
    const loadList = (page) => {
        ...
        http.post("/api/fankui/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
