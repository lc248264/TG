                    ...
                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                ...
                                <template v-if="row.shenqingzhuangtai == '待审核'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/ruzushenhe/add?id='+row.id)">入组审核</el-button>
                                </template>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/ruzushenqing/selectPagesChuangjianren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
