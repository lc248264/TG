<template>
    <div class="views-ruzushenqing-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 入组申请查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/ruzushenhe/add?id='+row.id)">添加入组审核</el-button>

                                <el-button type="default" :icon="Search" size="small" @click="$router.push('/admin/ruzushenhe?ruzushenqingid='+row.id)">入组审核查询</el-button>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/ruzushenqing/detail?id='+row.id)"></el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/ruzushenqing/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canRuzushenqingSelect, useRuzushenqingSelect, canRuzushenqingDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canRuzushenqingDelete(ids).then((res) => {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/ruzushenqing/selectPagesShenqingren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
