<template>
    <div class="views-zuzhixinxi-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 组织信息查询 </span>
                    </div>
                </template>
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="dizhi" label="地址" width="130">
                    ...
                    <el-table-column label="操作" fixed="right" width="280">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.shenhezhuangtai == '同意'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/zuzhihuodong/add?id='+row.id)">组织活动</el-button>
                                </template>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/zuzhixinxi/detail?id='+row.id)"></el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/zuzhixinxi/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canZuzhixinxiSelect, useZuzhixinxiSelect, canZuzhixinxiDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canZuzhixinxiDelete(ids).then((res) => {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/zuzhixinxi/selectPagesChuangjianren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
