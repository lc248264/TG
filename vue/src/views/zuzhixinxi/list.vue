                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.shenhezhuangtai == '待审核'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/zuzhishenhe/add?id='+row.id)">组织审核</el-button>
                                </template>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/zuzhixinxi/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
