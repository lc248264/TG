<template>
    <div class="views-huodongshenhe-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 活动审核查询 </span>
                    </div>
                </template>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/huodongshenhe/detail?id='+row.id)"></el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/huodongshenhe/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canHuodongshenheSelect, useHuodongshenheSelect, canHuodongshenheDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canHuodongshenheDelete(ids).then((res) => {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/huodongshenhe/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
