<template>
    <div class="views-jiesuanzhiyuan-list">
        <div>
            <el-card class="box-card">
                <template #header>
                    <div class="clearfix">
                        <span class="title"> 结算志愿查询 </span>
                    </div>
                </template>
                ...
                <div class="form-search">
                    <el-form @submit.prevent.stop :inline="true" size="small">
                        ...
                        <el-form-item label="完成情况">
                            <el-select v-model="search.wanchengqingkuang"
                                ><el-option label="请选择" value=""></el-option><el-option label="优秀" value="优秀"></el-option>
                        ...
                        <el-form-item label="审核状态">
                            <el-select v-model="search.shenhezhuangtai"
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="wanchengqingkuang" label="完成情况" width="80">
                        <template #default="{row}"> {{ row.wanchengqingkuang }} </template>
                    </el-table-column>
                    <el-table-column prop="shenhezhuangtai" label="审核状态" width="80">
                        <template #default="{row}"> {{ row.shenhezhuangtai }} </template>
                    </el-table-column>
                    <el-table-column prop="yijian" label="意见">
                        <template #default="{row}"> {{ row.yijian }} </template>
                    </el-table-column>
                    <el-table-column sortable="custom" prop="addtime" label="添加时间">
                    ...
                    <el-table-column label="操作" fixed="right" width="280">
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.shenhezhuangtai == '待审核'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/jiesuanshenhe/add?id='+row.id)">结算审核</el-button>
                                </template>

                                <el-tooltip effect="dark" content="详情" placement="top-start"
                                    ><el-button type="info" :icon="InfoFilled" size="small" @click="$router.push('/admin/jiesuanzhiyuan/detail?id='+row.id)"></el-button>
                                </el-tooltip>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/jiesuanzhiyuan/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canJiesuanzhiyuanSelect, useJiesuanzhiyuanSelect, canJiesuanzhiyuanDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canJiesuanzhiyuanDelete(ids).then((res) => {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/jiesuanzhiyuan/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
