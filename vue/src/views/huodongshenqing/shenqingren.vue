                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column label="操作" fixed="right" width="180">
                        <template #default="{row}">
                            <el-button-group>
                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/shenheshenqing/add?id='+row.id)">添加审核申请</el-button>

                                <el-button type="default" :icon="Search" size="small" @click="$router.push('/admin/shenheshenqing?huodongshenqingid='+row.id)"
                                    >审核申请查询</el-button
                                >

                                <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/jiesuanzhiyuan/add?id='+row.id)">添加结算志愿</el-button>

                                <el-button type="default" :icon="Search" size="small" @click="$router.push('/admin/jiesuanzhiyuan?huodongshenqingid='+row.id)"
                                    >结算志愿查询</el-button
                                >
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/huodongshenqing/selectPagesShenqingren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
