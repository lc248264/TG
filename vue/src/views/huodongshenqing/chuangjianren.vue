                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <template v-if="row.shenqingzhuangtai == '待审核'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/shenheshenqing/add?id='+row.id)">审核申请</el-button>
                                </template>
                                <template v-if="row.shenqingzhuangtai == '同意'">
                                    <el-button type="primary" :icon="Plus" size="small" @click="$router.push('/admin/jiesuanzhiyuan/add?id='+row.id)">结算志愿</el-button>
                                </template>
...
<script setup>
    ...
    const loadList = (page) => {
        ...
        http.post("/api/huodongshenqing/selectPagesChuangjianren", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
