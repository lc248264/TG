<template>
    <div class="views-zhiyuanzhe-list">
        <div>
            <el-card class="box-card">
                ...
                <el-table border :data="lists" style="width: 100%" @sort-change="sortChange" highlight-current-row>
                    ...
                    <el-table-column prop="yuanxi" label="院系" width="130">
                        <template #default="{row}"> {{ row.yuanxi }} </template>
                    </el-table-column>
                    <el-table-column prop="xuexiao" label="学校" width="130">
                        <template #default="{row}"> {{ row.xuexiao }} </template>
                    </el-table-column>
                    <el-table-column prop="gongyishizhang" label="公益时长" width="80">
                        <template #default="{row}"> {{ row.gongyishizhang }} </template>
                    </el-table-column>
                        ...
                        <template #default="{row}">
                            <el-button-group>
                                <el-tooltip effect="dark" content="编辑" placement="top-start"
                                    ><el-button type="success" :icon="Edit" size="small" @click="$router.push('/admin/zhiyuanzhe/updt?id='+row.id)"></el-button>
                                </el-tooltip>
...
<script setup>
    ...
    import { canZhiyuanzheSelect, useZhiyuanzheSelect, canZhiyuanzheDelete } from "@/module";
    ...
    const deleteItems = (ids) => {
        return new Promise((resolve, reject) => {
            ElMessageBox.confirm("确定删除？")
                .then((res) => {
                    canZhiyuanzheDelete(ids).then((res) => {
    ...
    const loadList = (page) => {
        ...
        http.post("/api/zhiyuanzhe/selectPages", search).then(
            (res) => {
                loading.value = false;
                if (res.code == 0) {
                    var data = res.data;
