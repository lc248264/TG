...
public class ZhiyuanzheService {
    ...
    @Resource
    private ZhiyuanzheMapper mapper;

    /**
     *   根据Xuehao字段参数获取一行数据
     */
    public Zhiyuanzhe findByXuehao(String username) {
        // 新建志愿者模块实体类Zhiyuanzhe
        Zhiyuanzhe pojo = new Zhiyuanzhe();
        // 设置参数
        pojo.setXuehao(username);
        // 根据实体类新建QueryWrapper查询条件类
        QueryWrapper<Zhiyuanzhe> queryWrapper = Wrappers.query(pojo);
        Zhiyuanzhe row = mapper.selectOne(queryWrapper);
        return row;
    }

    /**
     *   根据Xuehao字段参数获取一行数据，并不包含某uid 参数得行
     */
    public Zhiyuanzhe findByXuehao(String username, Integer uid) {
        // 新建志愿者模块实体类Zhiyuanzhe
        Zhiyuanzhe pojo = new Zhiyuanzhe();
        // 设置参数
        pojo.setXuehao(username);
     ...
     * 将数据转换成Session对象
     */
    public SessionUser toSession(Zhiyuanzhe user) {
        SessionUser session = new SessionUser();
        session.setId(user.getId());
        session.setUsername(user.getXuehao());
        session.setTable("zhiyuanzhe");
        session.setRoles("志愿者");
    ...
    public ResponseData<Zhiyuanzhe> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Zhiyuanzhe>> selectAll(Wrapper<Zhiyuanzhe> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Zhiyuanzhe>> selectAll() {
        QueryWrapper<Zhiyuanzhe> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Zhiyuanzhe> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Zhiyuanzhe> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Zhiyuanzhe> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("xuehao"))) {
            wrapper.like("xuehao", map.get("xuehao"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Zhiyuanzhe>> filter(Map<String, Object> map) {
        Class<Zhiyuanzhe> aClass = Zhiyuanzhe.class;
        com.jntoo.db.QueryWrapper<Zhiyuanzhe> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Zhiyuanzhe> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Zhiyuanzhe entityData, Map post) {
        if (findByXuehao(entityData.getXuehao()) != null) {
    ...
    public ResponseData<Object> update(Zhiyuanzhe entityData, Map post) {
        if (findByXuehao(entityData.getXuehao(), entityData.getId()) != null) {
            return JsonResult.error("账号已经存在");
        }
