...
@Service
public class ZuzhixinxiService {

    @Resource
    private ZuzhixinxiMapper mapper;

    public ResponseData<Zuzhixinxi> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Zuzhixinxi>> selectAll(Wrapper<Zuzhixinxi> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Zuzhixinxi>> selectAll() {
        QueryWrapper<Zuzhixinxi> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Zuzhixinxi> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Zuzhixinxi> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Zuzhixinxi> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( mingcheng LIKE '%" + map.get("keyword") + "%' OR jianjie LIKE '%" + map.get("keyword") + "%' OR dizhi LIKE '%" + map.get("keyword") + "%' ) ");
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Zuzhixinxi>> filter(Map<String, Object> map) {
        Class<Zuzhixinxi> aClass = Zuzhixinxi.class;
        com.jntoo.db.QueryWrapper<Zuzhixinxi> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Zuzhixinxi> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Zuzhixinxi entityData, Map post) {
    ...
    public ResponseData<Object> update(Zuzhixinxi entityData, Map post) {
