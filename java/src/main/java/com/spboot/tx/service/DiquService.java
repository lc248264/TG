...
public class DiquService {
    ...
    public ResponseData<Diqu> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Diqu>> selectAll(Wrapper<Diqu> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Diqu>> selectAll() {
        QueryWrapper<Diqu> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Diqu> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Diqu> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Diqu> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 以下是判断搜索框中是否有输入内容，判断是否前台是否有填写相关条件，符合则写入sql搜索语句
        if (!StringUtil.isNullOrEmpty(map.get("diquming"))) {
            wrapper.like("diquming", map.get("diquming"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Diqu>> filter(Map<String, Object> map) {
        Class<Diqu> aClass = Diqu.class;
        com.jntoo.db.QueryWrapper<Diqu> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Diqu> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Diqu entityData, Map post) {
    ...
    public ResponseData<Object> update(Diqu entityData, Map post) {
