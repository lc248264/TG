...
@Service
public class ZhiyuanqiangService {

    @Resource
    private ZhiyuanqiangMapper mapper;

    public ResponseData<Zhiyuanqiang> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Zhiyuanqiang>> selectAll(Wrapper<Zhiyuanqiang> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Zhiyuanqiang>> selectAll() {
        QueryWrapper<Zhiyuanqiang> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Zhiyuanqiang> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Zhiyuanqiang> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Zhiyuanqiang> wrapper = Wrappers.query();

        String where = " 1=1 ";
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(" ( wenti LIKE '%" + map.get("keyword") + "%' OR biaoti LIKE '%" + map.get("keyword") + "%' ) ");
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Zhiyuanqiang>> filter(Map<String, Object> map) {
        Class<Zhiyuanqiang> aClass = Zhiyuanqiang.class;
        com.jntoo.db.QueryWrapper<Zhiyuanqiang> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Zhiyuanqiang> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Zhiyuanqiang entityData, Map post) {
    ...
    public ResponseData<Object> update(Zhiyuanqiang entityData, Map post) {
