...
@Service
public class HuodongshenqingService {

    @Resource
    private HuodongshenqingMapper mapper;

    public ResponseData<Huodongshenqing> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Huodongshenqing>> selectAll(Wrapper<Huodongshenqing> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Huodongshenqing>> selectAll() {
        QueryWrapper<Huodongshenqing> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Huodongshenqing> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Huodongshenqing> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Huodongshenqing> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 判断URL 参数zuzhihuodongid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("zuzhihuodongid")) && Convert.toInt(map.get("zuzhihuodongid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("zuzhihuodongid", map.get("zuzhihuodongid"));
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("shenqingzhuangtai"))) {
            wrapper.eq("shenqingzhuangtai", map.get("shenqingzhuangtai"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Huodongshenqing>> filter(Map<String, Object> map) {
        Class<Huodongshenqing> aClass = Huodongshenqing.class;
        com.jntoo.db.QueryWrapper<Huodongshenqing> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Huodongshenqing> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Huodongshenqing entityData, Map post) {
    ...
    public ResponseData<Object> update(Huodongshenqing entityData, Map post) {
