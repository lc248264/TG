...
public class HuifuService {
    ...
    @Resource
    private HuifuMapper mapper;

    public ResponseData<Huifu> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Huifu>> selectAll(Wrapper<Huifu> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Huifu>> selectAll() {
        QueryWrapper<Huifu> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Huifu> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Huifu> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Huifu> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 判断URL 参数tieziid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("tieziid")) && Convert.toInt(map.get("tieziid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("tieziid", map.get("tieziid"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Huifu>> filter(Map<String, Object> map) {
        Class<Huifu> aClass = Huifu.class;
        com.jntoo.db.QueryWrapper<Huifu> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Huifu> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Huifu entityData, Map post) {
    ...
    public ResponseData<Object> update(Huifu entityData, Map post) {
