...
@Service
public class RuzushenheService {

    @Resource
    private RuzushenheMapper mapper;

    public ResponseData<Ruzushenhe> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Ruzushenhe>> selectAll(Wrapper<Ruzushenhe> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Ruzushenhe>> selectAll() {
        QueryWrapper<Ruzushenhe> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Ruzushenhe> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Ruzushenhe> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Ruzushenhe> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 判断URL 参数ruzushenqingid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("ruzushenqingid")) && Convert.toInt(map.get("ruzushenqingid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("ruzushenqingid", map.get("ruzushenqingid"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Ruzushenhe>> filter(Map<String, Object> map) {
        Class<Ruzushenhe> aClass = Ruzushenhe.class;
        com.jntoo.db.QueryWrapper<Ruzushenhe> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Ruzushenhe> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Ruzushenhe entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("update ruzushenqing set shenqingzhuangtai='" + post.get("shenhe") + "' where id='" + post.get("ruzushenqingid") + "'");
    ...
    public ResponseData<Object> update(Ruzushenhe entityData, Map post) {
