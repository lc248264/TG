...
@Service
public class ZuzhihuodongService {

    @Resource
    private ZuzhihuodongMapper mapper;

    public ResponseData<Zuzhihuodong> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Zuzhihuodong>> selectAll(Wrapper<Zuzhihuodong> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Zuzhihuodong>> selectAll() {
        QueryWrapper<Zuzhihuodong> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Zuzhihuodong> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Zuzhihuodong> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Zuzhihuodong> wrapper = Wrappers.query();
        ...
        if (!StringUtil.isNullOrEmpty(map.get("keyword"))) {
            wrapper.apply(
                ...
                "%' OR mingcheng LIKE '%" +
                map.get("keyword") +
                "%' OR huodongzhuti LIKE '%" +
                map.get("keyword") +
                "%' OR huodongjieshao LIKE '%" +
                map.get("keyword") +
                "%' OR huodongdizhi LIKE '%" +
                map.get("keyword") +
                "%' ) "
            );
        }
        // 判断URL 参数zuzhixinxiid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("zuzhixinxiid")) && Convert.toInt(map.get("zuzhixinxiid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("zuzhixinxiid", map.get("zuzhixinxiid"));
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("shenhezhuangtai"))) {
            wrapper.eq("shenhezhuangtai", map.get("shenhezhuangtai"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Zuzhihuodong>> filter(Map<String, Object> map) {
        Class<Zuzhihuodong> aClass = Zuzhihuodong.class;
        com.jntoo.db.QueryWrapper<Zuzhihuodong> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Zuzhihuodong> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Zuzhihuodong entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("UPDATE zuzhihuodong SET huodongshizhang=(TIMESTAMPDIFF(HOUR , kaishishijian,jieshushijian)) WHERE id='" + charuid + "'");
    ...
    public ResponseData<Object> update(Zuzhihuodong entityData, Map post) {
        mapper.updateById(entityData);

        Integer charuid = entityData.getId();
        DB.execute("UPDATE zuzhihuodong SET huodongshizhang=(TIMESTAMPDIFF(HOUR , kaishishijian,jieshushijian)) WHERE id='" + post.get("id") + "'");
