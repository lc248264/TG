...
public class DianzanService {
    ...
    public ResponseData<Dianzan> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Dianzan>> selectAll(Wrapper<Dianzan> query) {
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Dianzan>> filter(Map<String, Object> map) {
        Class<Dianzan> aClass = Dianzan.class;
        com.jntoo.db.QueryWrapper<Dianzan> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Dianzan> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Dianzan entityData, Map post) {
        Info.handlerNullEntity(entityData);
        entityData.setUsername(Request.user().getUsername());
        entityData.setAddtime(Info.getDateStr());

        Map res = DB.name("dianzan").where("biao", post.get("biao")).where("biaoid", post.get("biaoid")).where("username", Request.user().getUsername()).find();
        if (res != null && !res.isEmpty()) {
            DB.name("dianzan").where("id", res.get("id")).delete();
            return JsonResult.success("已取消点赞");
        }
    ...
    public ResponseData<Object> update(Dianzan entityData, Map post) {
