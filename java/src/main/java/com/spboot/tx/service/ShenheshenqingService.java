...
@Service
public class ShenheshenqingService {

    @Resource
    private ShenheshenqingMapper mapper;

    public ResponseData<Shenheshenqing> findById(Integer id) {
        return JsonResult.success(mapper.selectById(id));
    }

    public ResponseData<List<Shenheshenqing>> selectAll(Wrapper<Shenheshenqing> query) {
        return JsonResult.success(mapper.selectList(query));
    }

    public ResponseData<List<Shenheshenqing>> selectAll() {
        QueryWrapper<Shenheshenqing> wrapper = Wrappers.query();
    ...
    public ResponseData selectAll(Map<String, Object> map) {
        ...
        QueryWrapper<Shenheshenqing> wrapper = mapToWrapper(map);
    ...
    public QueryWrapper<Shenheshenqing> mapToWrapper(Map<String, Object> map) {
        QueryWrapper<Shenheshenqing> wrapper = Wrappers.query();

        String where = " 1=1 ";
        // 判断URL 参数huodongshenqingid是否大于0
        if (!StringUtil.isNullOrEmpty(map.get("huodongshenqingid")) && Convert.toInt(map.get("huodongshenqingid")) > 0) {
            // 大于0 则写入条件
            wrapper.eq("huodongshenqingid", map.get("huodongshenqingid"));
        }
        ...
        if (!StringUtil.isNullOrEmpty(map.get("dianhua"))) {
            wrapper.like("dianhua", map.get("dianhua"));
        }
     ...
     * @return 搜索到得数据
     */
    public ResponseData<List<Shenheshenqing>> filter(Map<String, Object> map) {
        Class<Shenheshenqing> aClass = Shenheshenqing.class;
        com.jntoo.db.QueryWrapper<Shenheshenqing> wrapper = DB.name(aClass);
    ...
    public ResponseData selectPages(QueryWrapper<Shenheshenqing> wrapper, IPage page) {
        Map result = new HashMap();
        result.put("lists", mapper.selectPage(page, wrapper));
        return JsonResult.success(result);
    }

    public ResponseData insert(Shenheshenqing entityData, Map post) {
        ...
        if (entityData.getId() != null) {
            ...
            DB.execute("update huodongshenqing set shenqingzhuangtai='" + post.get("shenhe") + "' where id='" + post.get("huodongshenqingid") + "'");
            DB.execute("update zuzhihuodong set huodongrenshu=huodongrenshu-1 where id='"+post.get("zuzhihuodongid")+"' and '"+post.get("shenhe")+"'='同意'");
    ...
    public ResponseData<Object> update(Shenheshenqing entityData, Map post) {
